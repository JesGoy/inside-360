...
export default function Home() {
  ...
  const onPointerMove = useCallback((event: PointerEvent) => {
    ...
    lonRef.current = (onPointerDownMouseXRef.current - event.clientX) * 0.1 + onPointerDownLonRef.current;
    latRef.current = (event.clientY - onPointerDownMouseYRef.current) * 0.1 + onPointerDownLatRef.current;
  ...
  const onTouchMove = useCallback((event: TouchEvent) => {
    if (event.touches.length === 1) {
      event.preventDefault();
      lonRef.current = (onPointerDownMouseXRef.current - event.touches[0].pageX) * 0.1 + onPointerDownLonRef.current;
      latRef.current = (event.touches[0].pageY - onPointerDownMouseYRef.current) * 0.1 + onPointerDownLatRef.current;
      latRef.current = Math.max(-85, Math.min(85, latRef.current));
    }
  }, []);
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      cameraRef.current = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 1, 1100);
  ...
  }, [animate, onWindowResize, onPointerDown, onDeviceOrientation, onTouchStart, onTouchMove]);
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      <Image src="/images/logo.svg" alt="Logo" width={100} height={100} style={{ position: 'absolute', top: 10, left: 10 }} />
      {showGyroscopeButton && (
        <button
